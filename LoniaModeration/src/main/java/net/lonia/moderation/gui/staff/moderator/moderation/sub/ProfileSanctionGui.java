package net.lonia.moderation.gui.staff.moderator.moderation.sub;

import net.lonia.core.LoniaCore;
import net.lonia.core.item.builder.ItemBuilder;
import net.lonia.core.item.manager.ItemManager;
import net.lonia.core.player.account.Account;
import net.lonia.core.player.sanction.SanctionManager;
import net.lonia.core.utils.GuiBuilder;
import net.lonia.moderation.LoniaModeration;
import net.lonia.moderation.gui.staff.moderator.moderation.ModerationGui;
import net.lonia.moderation.manager.Sanctions;
import org.bukkit.Material;
import org.bukkit.entity.Player;
import org.bukkit.inventory.Inventory;
import org.bukkit.inventory.InventoryView;
import org.bukkit.inventory.ItemStack;
import org.bukkit.inventory.meta.ItemMeta;

import java.util.UUID;

import static net.lonia.core.item.manager.ModerationItemManager.*;
import static net.lonia.moderation.manager.Sanctions.getCurrentDate;

public class ProfileSanctionGui implements GuiBuilder {

    public static final ItemStack profileIco = new ItemBuilder(Material.NAME_TAG).setName("§cProfil").setLore("§65 sanctions-type").toItemStack();


    @Override
    public String name() {
        return "§c§l‣ Profil";
    }

    @Override
    public int getSize() {
        return 27;
    }

    @Override
    public void contents(Player player, Inventory inv) {
        inv.setItem(0, profileIco);

        inv.setItem(11, invalidPseudo);
        inv.setItem(12, invalidSkinI);
        inv.setItem(13, invalidSkinII);
        inv.setItem(14, invalidStatusI);
        inv.setItem(15, invalidStatusII);
        inv.setItem(26, ItemManager.RETURN);
    }

    @Override
    public void onClick(Player player, Inventory inv, InventoryView invView, ItemStack current, int slot) {
        if (!invView.getTitle().equals(name()))
            return;

        if (!current.hasItemMeta())
            return;
        final ItemMeta im = current.getItemMeta();

        if (!im.hasDisplayName())
            return;
        final String displayName = im.getDisplayName();

        final String targetName = LoniaCore.get().getAccountManager().getAccount(player).getDataModeration().getReportedPlayer();
        final UUID targetUUID = Account.getUUID(targetName);

        if (displayName.equals(invalidPseudo.getItemMeta().getDisplayName())) {
            SanctionManager.ban(targetUUID, targetName, Sanctions.BAD_USERNAME.getDuration(), getCurrentDate(), player.getDisplayName(), Sanctions.BAD_USERNAME.getName());
        } else if (displayName.equals(invalidSkinI.getItemMeta().getDisplayName())) {
            SanctionManager.ban(targetUUID, targetName, Sanctions.BAD_SKIN.getDuration(), getCurrentDate(), player.getDisplayName(), Sanctions.BAD_SKIN.getName());
        } else if (displayName.equals(invalidSkinII.getItemMeta().getDisplayName())) {
            SanctionManager.ban(targetUUID, targetName, Sanctions.BAD_SKIN2.getDuration(), getCurrentDate(), player.getDisplayName(), Sanctions.BAD_SKIN2.getName());
        } else if (displayName.equals(invalidStatusI.getItemMeta().getDisplayName())) {
            SanctionManager.ban(targetUUID, targetName, Sanctions.BAD_STATUS.getDuration(), getCurrentDate(), player.getDisplayName(), Sanctions.BAD_STATUS.getName());
        } else if (displayName.equals(invalidStatusII.getItemMeta().getDisplayName())) {
            SanctionManager.ban(targetUUID, targetName, Sanctions.BAD_STATUS2.getDuration(), getCurrentDate(), player.getDisplayName(), Sanctions.BAD_STATUS2.getName());
        } else if (displayName.equals(ItemManager.RETURN.getItemMeta().getDisplayName()))
            LoniaModeration.getInstance().getGuiManager().open(player, ModerationGui.class);
    }
}
