...
public class UnMuteGUI implements GuiBuilder {
    ...
    public void contents(Player player, Inventory inv) {
        ...
        final String time = formatDuration(SanctionManager.getMuteRemainingTime(uuid));
        final Rank rank = Account.getRank(uuid);
        final String reason = SanctionManager.getMuteReason(uuid);
        final String message = SanctionManager.getMessage(uuid);
        ...
        if (message.equals("CUSTOM"))
            ...
            inv.setItem(0, new ItemBuilder(Material.PAPER).setName("§c" + reason + "(" + time + ")").setLore(rank.getPrefix() + targetName, "§f" + message).toItemStack());
    ...
    public void onClick(Player player, Inventory inv, InventoryView invView, ItemStack current, int slot) {
        ...
        if (displayName.equals(ERRORJUGE.getItemMeta().getDisplayName())) {
            SanctionManager.unmute(uuid, "Erreur de jugement");
            player.sendMessage(SanctionMessages.unmute());
            player.closeInventory();
        } else if (displayName.equals(ERRORNAME.getItemMeta().getDisplayName())) {
            SanctionManager.unmute(uuid, "Erreur de pseudonyme");
            player.sendMessage(SanctionMessages.unmute());
            player.closeInventory();
