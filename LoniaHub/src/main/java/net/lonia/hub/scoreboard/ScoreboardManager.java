...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = LoniaHub.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() -> {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                LoniaHub.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = LoniaHub.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() -> {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                LoniaHub.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
